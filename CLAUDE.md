...
Slackメッセージをキーワードで検索します（`search:read`スコープが必要）。
...
検索結果には各メッセージのpermalinkが含まれ、`get_slack_thread`でスレッド全体を取得できます。
...
- **クリップボード自動コピー**: `draft_reply` で添削した後、コピー用テキストを `pbcopy` でクリップボードに自動コピーする
